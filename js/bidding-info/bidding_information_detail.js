...
var collectiontype = "";
function getData(id) {
    $.getJSON(baseurl + "/system/tenderingInfo/" + id, function(data) {
        console.log('招标信息的公告',data);
      ...
      $.ajax({
        ...
        data: {infoType:"招标信息",title: data.data.name}, //请求的参数 = 给后台传的参数
...
function collectproject(infoType, location, title, infoId) {
    ...
    obj = collectiontype == "已收藏"?{id:infoId,infoType:'招标信息'}: {
      folderType: infoType,
      region: location,
      title: title,
      infoId: infoId
