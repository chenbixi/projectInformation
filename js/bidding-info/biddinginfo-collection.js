...
function renderCollect(element, data, total, index) {
    ...
    $("#bidding-collection-list").html(htmlStr);
    var html = '';
    ...
    $("#bidding-collection-list").append(html + ' </tbody>');
...
$.getJSON(baseurl + "/index/region", function (data) {
    ...
    for (var i = 0; i < data.data.length; i++) {
        ...
        $("#bidding_collection_list").append(
            "<option value='" +
            data.data[i].regionName +
            "'>" +
            data.data[i].regionName +
 ...
 * 条件搜索
 */
function search_bidding_collection(){
    region = $("#bidding_collection_list").val();
    name = $("#search_bidding_title").val();
    getCollectionData(
        {
            region: region,
            title: name,
