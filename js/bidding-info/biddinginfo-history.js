...
function getCollectionData(obj, callback) {
    $.ajax({
        type: "post",
        url: baseurl + "/system/looksHistory/list",
        data: obj,
        dataType: "json",
...
function renderCollect(element, data, total, index) {
    ...
    var htmlStr = ' <thead><tr class=\"success\"><th></th><th>标题</th><th>地区</th><th>浏览时间</th></tr></thead><tbody id=\"projectinforlist\">';
    ...
    for (var i = 0, length = data.length; i < length; i++) {
        ...
        html +=
            ...
            "<td  ><a  style='display: inline-block;text-overflow: ellipsis;overflow: hidden;white-space: nowrap;width: 377px;' target='_blank' href='../../../jgw/bidding_information_detail.html?id=" + data[i].infoId + " '> " +
            data[i].title +
            "</a></td>" +
            "<td style=''>" +
            data[i].region +
    ...
    $("#history-list").append(html + ' </tbody>');
 ...
 * 条件搜索
 */
function search_project_history(){
    region = $("#project_history_list").val();
    name = $("#search_history_title").val();
    getCollectionData(
        {
            region: region,
            title: name,
