...
var collectiontype = "";
function getData(id) {
    $.getJSON(baseurl + "/system/landMarket/" + id, function(data) {
        console.log(data);
      ...
      $.ajax({
        ...
        data: {infoType:"土地交易",title: data.data.title}, //请求的参数 = 给后台传的参数
...
function printHtml(html) {
  ...
  function collectproject(infoType, location, title, infoId) {
    ...
    obj = collectiontype == "已收藏"?{id:infoId,infoType:'土地交易'}: {
      folderType: infoType,
      region: location,
      title: title,
      infoId: infoId
