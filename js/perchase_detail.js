  ...
  $.getJSON(baseurl + "/system/purchaseInfo/" + id, function(data) {
    ...
    if (data.code == -100) {
      ...
      $.ajax({
        ...
        data: {infoType:"采购信息",title: data.data.title}, //请求的参数 = 给后台传的参数
...
<!-- 信息详情的标题结束 -->
<!-- 正文 -->
<div class="main_body" style="height: 80%;overflow-y:scroll; overflow-x:auto;height:700px;">
    <div class="body_page" style=" overflow-y:auto" id="printArea">
    <table id="process-demo-2" class="tb tb-b c-1" style="margin:10px auto;margin-left:6%;width:100%">
        <thead>
        </thead>
        <tbody>
        <tr>
        ...
        <tr>
            <td class="titla_name">采购类型</td>
            <td style="width: 200px"> ${data.data.type}</td>
        ...
        <tr>
            ...
            `;
      $("#purchase_content").html(finalData);
      // document.getElementById("purchase_content").innerHTML = finalData;
...
function collectproject(infoType, location, title, infoId) {
  ...
  obj = collectiontype == "已收藏"?{id:infoId,infoType:'采购信息'}: {
    folderType: infoType,
    region: location,
    title: title,
    infoId: infoId
